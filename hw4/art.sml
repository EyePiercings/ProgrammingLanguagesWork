...
fun build (depth, rand: RandomGenerator) =
    let
	fun cExpr (a, 0) = if rand(0,1) > 0 then buildX() else buildY()
	  | cExpr (1, x) = buildX() 
	  | cExpr (2, x) = buildY()
	  | cExpr (3, x) = buildSine(cExpr (rand(1,10), (x-1)))
	  | cExpr (4, x) = buildSine(cExpr (rand(1,10),  (x-1)))
	  | cExpr (5, x) = buildCosine(cExpr (rand(1,10), (x-1)))
	  | cExpr (6, x) = buildCosine(cExpr (rand(1,10), (x-1)))
	  | cExpr (7, x) = buildAverage(cExpr (rand(1,10), (x-1)),cExpr (rand(1,10), (x-1)))
	  | cExpr (8, x) = buildAverage(cExpr (rand(1,10), (x-1)),cExpr (rand(1,10), (x-1)))
	  | cExpr (9, x) = buildTimes(cExpr (rand(1,10), (x-1)),cExpr (rand(1,10), (x-1)))
	  | cExpr (10, x) = buildTimes(cExpr (rand(1,10), (x-1)),cExpr (rand(1,10), (x-1)))	
    in
	cExpr (rand(1,10), depth)
    end;
...
let val rand = makeRand(12,40) in 
    build(12, rand)
end
