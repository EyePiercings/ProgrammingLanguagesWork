...
object TextEditor {
  ...
  def move(count: Int) = {
    ...
    /** Checks if we are in the middle of an undo */
    ...
    if(comArray.size > 1) {
      val oldC = comArray(index-1)
      println("Index: " + index + " comArray.size: " + comArray.size)
      println("oldC: " + oldC.toString())

      command.compose(oldC) match {
	case None =>  comArray += command; command.execute(); // Can't combine them
	case Some(newC) =>  undo(); comArray -= oldC; comArray += newC; newC.execute();
      }
      println("Index: " + index + " comArray.size: " + comArray.size)
    } else {
      comArray += command
  ...
  def insert(text: String) = {
    ...
    if(comArray.size >= 1) {
      ...
      undo()
      command.compose(oldC) match {
	case None =>  redo(); comArray += command; command.execute(); // Can't combine them
	case Some(newC) => comArray -= oldC; comArray += newC; newC.execute();
      }
    } else {
      comArray += command
      command.execute()
  ...
  def delete(count: Int) =  {
    ...
    if(comArray.size > 1) {
      ...
      undo()
      command.compose(oldC) match {
	case None =>  println("None");redo(); comArray += command; command.execute(); // Can't combine them
	case Some(newC) =>  comArray -= oldC; comArray += newC; newC.execute();
      }
      println("Index: " + index + " comArray.size: " + comArray.size)
    } else {
  ...
  /**
   * Redo the next previous editing command
   * (either a move, insert, or delete).
   */
  def redo() = {
